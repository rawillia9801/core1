...
- `core_phone_lookup_matches_view`: one row per distinct buyer or family-linked profile match for a normalized phone, with IDs for authorized server/staff ambiguity handling and no puppy/payment/go-home context.
...
The read model intentionally uses nullable relationships because imported or newly received customer records may be incomplete.
...
- Matched buyer/family IDs in the summary/match views are for future authorized server/staff use only, not customer or public disclosure.
...
-> if ambiguous, route to staff or request approved additional verification
...
- Ambiguous results require verification or staff routing before exposing transaction details.
